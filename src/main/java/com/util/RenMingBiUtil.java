...
public class RenMingBiUtil {
	...
	private String transInt(String str) {
		...
		for (int i = 0; i < len; i++) {
			 ...
			 * 但我希望得到的是后面这种，因为发票所需显示的就是后面这种 if (firstFourBitRmb[i] == '0' && i <
			 * len) { boolean zeroFlag = false; while (i < len &&
			 * firstFourBitRmb[i] == '0') { zeroFlag = true; i++; } if (i != len
			 * && zeroFlag == true) strResult += "零"; i--; } else { int m =
			 * firstFourBitRmb[i] - '0'; strResult += strHan[m] + moneyCount[len
			 * - i - 1]; }
			 **/
